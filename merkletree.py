...
class MarkleTree:
    ...
    def print_tree(self, hash):
        ...
        #     #print(value[0])
        #     return
        # list_to_process1 = value[1]
        # list_to_process = list_to_process1.keys()
        # for x in list_to_process:
        #     if self._transactions[x] != None:
        #         print(x + " " + self._transactions[x][0])
        #         self.print_tree( x)
        #     else:
        #         print(str(x) + " " + list_to_process1[x])
        # return 
        list_to_process1 = value[1]
        items = value[0]
        print("%s %s" % (hash, items))
        if list_to_process1 == None:
            return
        for it_hash , item in list_to_process1.items():
            print("-> %s %s" % (it_hash, item))
        for it_hash , item in list_to_process1.items():
            self.print_tree(it_hash)
...
def find_diff(merkle_tree_a, a_roothash, merkle_tree_b, b_roothash):
    if a_roothash == b_roothash:
        ...
        # TODO
        #print("todo")
        for it_hash, item in a_child.items():
            try:
                if b_child[it_hash] == item:
                    print("SAME : %s" % item)
            except:
                print("DIFF : %s" % item)
                temp_v = merkle_tree_a._transactions[it_hash]
                if len(temp_v[1]) > 0:      
                    diffhash = list(set(b_child.keys()) - set(a_child.keys()))
                    find_diff(merkle_tree_a, it_hash, merkle_tree_b, diffhash[0])
                
if __name__ == "__main__":
    merkle_tree_a = MarkleTree('A')

    merkle_tree_b = MarkleTree('B')
    # move the files to program files for debugging purpose
    # merkle_tree_a = MarkleTree('c:\\Users\\Madhukar\\Downloads\\cmpe273-fall18-master\\exams\\final\\merkle-tree\\A')
    # merkle_tree_b = MarkleTree('c:\\Users\\Madhukar\\Downloads\\cmpe273-fall18-master\\exams\\final\\merkle-tree\\B')
